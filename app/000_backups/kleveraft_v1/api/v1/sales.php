...
function _post(){
    ...
    $query = "INSERT INTO `sales` (total_qty, total_amt, transaction_date, comment, modified_by, date_modified) VALUES ('". $total_qty ."','". $total_amt ."','". $transaction_date ."','". $comment ."','". $modified_by ."', '".$server_time."')";
...
function _get(){
    ...
    $fPartner = '';
        ...
        if((isset($_GET['startdate']) && !empty($_GET['startdate'])) && (isset($_GET['enddate']) && !empty($_GET['enddate']))){
            ...
            $fTransactionDate = "(date(f.transaction_date) BETWEEN '". $startdate ."' AND '" . $enddate . "')";
        }
    ...
    */
        ...
        if(isset($_GET['id']) && !empty($_GET['id'])){ // read with breakdown
            ...
            $query = "SELECT id as sales_id, total_qty, total_amt, date(transaction_date) as transaction_date, comment FROM `sales` WHERE id = '" . $sales_id . "'";
            ...
            $query = "SELECT id as sales_id, total_qty, total_amt, date(transaction_date) as transaction_date, comment FROM `sales`";
            $qresult = mysqli_query($nect, $query);
