...
function _get(){
    ...
    if(isset($_GET['mode']) && !empty($_GET['mode'])){
        ...
        if($mode == 'partner'){
            ...
            $qGraph = "SELECT date(f.transaction_date) as transaction_date, COALESCE(SUM(a.total), 0.00) as total_amount FROM `sales_breakdown` as a INNER JOIN `products` as b ON (a.product_id = b.id) INNER JOIN `client_products` as c ON (b.id=c.product_id) INNER JOIN `clients` as d ON (c.partner_id = d.id) INNER JOIN `product_categories` as e ON (b.category_id = e.id) INNER JOIN `sales` as f ON (a.sales_id = f.id) WHERE d.id = '".$partner_id."' AND (date(f.transaction_date) BETWEEN '". $startdate ."' AND '" . $enddate . "') GROUP BY date(f.transaction_date)";
            ...
            if($qresult){
                if(mysqli_num_rows($qresult) != 0){
                    while($fetchResult = mysqli_fetch_array($qresult)){
                        $transaction_date = $fetchResult['transaction_date'];
                        $total_amt = $fetchResult['total_amount'];
                        
                        $sales_array['transaction_date'] = $transaction_date;
                ...
                foreach($daterange as $date){
                    $graph_array['transaction_date'] = $date -> format("Y-m-d");
                    $total = '0.00';
                    foreach($weeklysales as $x){
                        if($date -> format("Y-m-d") == $x["transaction_date"]){    
