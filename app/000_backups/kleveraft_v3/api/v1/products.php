...
function _get(){
    ...
    if(isset($_GET['id']) && !empty($_GET['id'])){ // read
        ...
        $query = "SELECT a.id as product_id, b.name as category, CONCAT(c.url, c.location, c.file_name) as product_image, a.name as product_name, a.code, a.description, a.qty, a.price, e.name as partner_name FROM `products` as a INNER JOIN `product_categories` as b ON (a.category_id = b.id) LEFT JOIN `files` as c ON (a.image_id = c.id) INNER JOIN `client_products` as d ON (a.id = d.product_id) INNER JOIN `clients` as e ON (d.partner_id = e.id) WHERE ". $partnerCondition ."  a.id = '". $product_id ."' AND a.status = 1";
        
    } else { // show all
        $query = "SELECT a.id as product_id, b.name as category, CONCAT(c.url, c.location, c.file_name) as product_image, a.name as product_name, a.code, a.description, a.qty, a.price, e.name as partner_name FROM `products` as a INNER JOIN `product_categories` as b ON (a.category_id = b.id) LEFT JOIN `files` as c ON (a.image_id = c.id) INNER JOIN `client_products` as d ON (a.id = d.product_id) INNER JOIN `clients` as e ON (d.partner_id = e.id) WHERE ". $partnerCondition ." a.status = 1";
    }
